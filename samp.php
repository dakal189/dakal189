...
function replyMainKeyboard(string $lang, bool $isAdmin): array { $kb=[[t('skins',$lang),t('vehicles',$lang)],[t('weapons',$lang),t('objects',$lang)],[t('mapping',$lang),t('colors',$lang)],[t('weather',$lang),t('rules',$lang)],[t('favorites',$lang),t('language',$lang)]]; if($isAdmin)$kb[]=[t('panel',$lang)]; return ['keyboard'=>array_map(fn($row)=>array_map(fn($t)=>['text'=>$t],$row),$kb),'resize_keyboard'=>true,'one_time_keyboard'=>false]; }
...
function handleText(array $msg): void {
	...
	if ($session && $session['state']==='channel_add') { $parts=preg_split('/\s+/', $text); $cid=(int)($parts[0]??0); $username=null; foreach($parts as $p2) if(str_starts_with($p2,'@')) $username=ltrim($p2,'@'); $title=null; if(count($parts)>1){ $rest=array_slice($parts,1); $rest=array_values(array_filter($rest,fn($x)=>!str_starts_with($x,'@'))); $title=implode(' ', $rest); } if($cid){ db()->prepare('INSERT INTO required_channels (chat_id,username,title) VALUES (?,?,?)')->execute([$cid,$username,$title]); sendMessage($chatId, t('saved',$lang)); } clearSession($userId); return; }
